...
async def register_doctor_route(
    ...
    try:
        ...
        await logs_service.add_logs(action="GENERATE_CSR", log_level="INFO", user_id=user_info.username, user_role="doctor", patient_id="null", message="CSR generated successfully")
        time.sleep(5)
        cert_path = auth_service.check_csr_signed(user_info.username)
        await logs_service.add_logs(action="CHECK_CSR_SIGNED", log_level="INFO", user_id=user_info.username, user_role="doctor", patient_id="null", message="CSR signed and retrieved successfully")
        p12_password = auth_service.create_doctor_in_keycloak(cert_path, user_info)
        await logs_service.add_logs(action="REGISTER_DOCTOR", log_level="INFO", user_id=user_info.username, user_role="doctor", patient_id="null", message="Doctor registered in Keycloak")
        p12_content = file_service.get_base64_file_content(cert_path)
        await logs_service.add_logs(action="GET_P12_CONTENT", log_level="INFO", user_id=user_info.username, user_role="doctor", patient_id="null", message="P12 certificate content retrieved")
