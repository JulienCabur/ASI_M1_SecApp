...
@router.post("/store_user_keys", response_model=Dict[str, Any])
def store_user_keys_route(
    ...
    try:
        ...
        key_service.store_user_keys(user_id=current_user.id, public_key=key_data.public_key, ciphered_kek=key_data.ciphered_kek)
        logs_service.add_logs(action="STORE_USER_KEYS", log_level="INFO", user_id=current_user.id, user_role="unknown", patient_id="null")
        return {"status": "Clés stockées avec succès pour l'utilisateur"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Erreur lors du stockage des clés: {str(e)}")

@router.get("/get_user_keys", response_model=KeyBase)
def get_user_keys_route(
    ...
    try:
        key_service = KeyService(db=db)
        user_keys = key_service.get_user_keys(user_id=current_user.id)
        logs_service.add_logs(action="GET_USER_KEYS", log_level="INFO", user_id=current_user.id, user_role="unknown", patient_id="null")
        return user_keys
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Erreur lors de la récupération des clés: {str(e)}")

@router.get("/get_user_public_key", response_model=Dict[str, str])
def get_user_public_key_route(
    ...
    try:
        key_service = KeyService(db=db)
        user_keys = key_service.get_user_keys(user_id=target_user_id)
        logs_service.add_logs(action="GET_USER_PUBLIC_KEY", log_level="INFO", user_id=current_user.id, user_role="unknown", patient_id="null")
